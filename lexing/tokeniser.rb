...
class Tokeniser
    ...
    def scan_token
        ...
        when 'τ'
            add_token(:REAL, Math::PI * 2)
        when 'π'
            add_token(:REAL, Math::PI)
        when 'ℯ'
            add_token(:REAL, Math::E)
    ...
    def number
        ...
        if peek == '.' && peek_next =~ /\d/
            ...
            add_token(:REAL, @source[@start...@current].gsub("_", "").to_f)
        elsif peek == '/' && peek_next =~ /\d/
            advance
            advance while peek =~ /\d/
            add_token(:RATIONAL, @source[@start...@current].gsub("_", "").to_r)
        else
            add_token(:INTEGER, @source[@start...@current].gsub("_", "").to_i)
        end
    end
